# Receptkwaliteit Verbeteringen Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Voeg kooktijd en genummerde bereidingsstappen toe aan recepten, en voorkom herhaling van de laatste 20 recepten door de AI.

**Architecture:** Task 1 past de AI-prompt en handler in `generate-meal-plan.js` aan: nieuw schema (`cookingTime`, `preparationSteps` array i.p.v. `preparation` string) plus een KV-lookup voor receptgeschiedenis vóór de AI-call. Task 2 past de frontend aan om de nieuwe velden te tonen met achterwaartse compatibiliteit voor eerder opgeslagen plannen.

**Tech Stack:** Node.js serverless (Vercel), @anthropic-ai/sdk, @vercel/kv, vanilla JS

---

### Task 1: Backend — nieuw schema + kooktijd + stappen + herhaling-check

**Files:**
- Modify: `api/generate-meal-plan.js`

- [ ] **Stap 1: Vervang de volledige inhoud van `api/generate-meal-plan.js`**

Schrijf het bestand als volgt (volledige vervanging):
...
- [ ] **Stap 2: Verifieer het bestand**

Check visueel:
- `const { kv } = require('@vercel/kv');` staat bovenaan (regel 2)
- `cookingTime` en `preparationSteps` staan in het JSON-schema voorbeeld
- `preparation` staat NIET meer in het JSON-schema voorbeeld
- `recentNames` try/catch staat vóór de `userMessage` definitie
- `recentNames.length > 0 ? ...` staat in de `userMessage` template literal
- Template literal voor `SYSTEM_PROMPT` is intact (opent op regel 4, sluit na VEILIGHEID)

- [ ] **Stap 3: Commit**

```bash
git add api/generate-meal-plan.js
git commit -m "feat(prompt): add cookingTime, preparationSteps array, and recipe dedup from KV history"
```

---

### Task 2: Frontend — mapping + renderMealPlan + loadRecepten

**Files:**
- Modify: `public/index.html`

- [ ] **Stap 1: Update de `mealPlan` mapping in `doPlan()` (huidige regels 386–395)**
...
```javascript
    mealPlan = data.meals.map(m => ({
      ...
      preparationSteps: m.preparationSteps || [],
...
- [ ] **Stap 2: Update `renderMealPlan()` — vervang de preparation-regel en voeg cookingTime toe**

De huidige `renderMealPlan()` heeft op regel ~344–352:
```javascript
...
Vervang die twee regels (meal-name + preparation) door:

```javascript
      <div class="meal-name">${esc(m.name)}</div>
      ${m.cookingTime ? `<div style="font-size:12px;color:#888;margin-top:2px;">⏱ ${esc(m.cookingTime)}</div>` : ''}
```

En vervang de `preparation`-regel door de stappen-logica:

```javascript
      ${m.preparationSteps && m.preparationSteps.length > 0
        ? `<ol style="margin:6px 0 0 16px;padding:0;font-size:13px;color:#444;">${m.preparationSteps.map(s => `<li style="margin-bottom:3px;">${esc(s)}</li>`).join('')}</ol>`
        : m.preparation ? `<div class="meal-ing" style="margin-top:6px;color:#444;">${esc(m.preparation)}</div>` : ''}
```

- [ ] **Stap 3: Update `loadRecepten()` — cookingTime en stappen in het `<details>` blok**

De huidige `loadRecepten()` heeft op regel ~498:
```javascript
            ${m.preparation ? `<div class="meal-ing" style="margin-top:6px;color:#444;">${esc(m.preparation)}</div>` : ''}
```

Voeg cookingTime toe vóór de bereidingslijn en vervang de preparation-regel:

```javascript
            ${m.cookingTime ? `<div style="font-size:12px;color:#888;margin-bottom:4px;">⏱ ${esc(m.cookingTime)}</div>` : ''}
            ${m.preparationSteps && m.preparationSteps.length > 0
              ? `<ol style="margin:4px 0 0 16px;padding:0;font-size:13px;color:#444;">${m.preparationSteps.map(s => `<li style="margin-bottom:3px;">${esc(s)}</li>`).join('')}</ol>`
              : m.preparation ? `<div class="meal-ing" style="margin-top:6px;color:#444;">${esc(m.preparation)}</div>` : ''}
```

- [ ] **Stap 4: Verifieer de wijzigingen**

Check visueel in `public/index.html`:
- `cookingTime: m.cookingTime || ''` staat in de mealPlan mapping
- `preparationSteps: m.preparationSteps || []` staat in de mealPlan mapping
- `preparation: m.preparation || ''` staat nog steeds in de mapping (backward compat)
- `renderMealPlan()`: cookingTime badge staat onder meal-name, stappen-logica valt terug op `m.preparation` als array leeg is
- `loadRecepten()`: zelfde patroon in de `<details>` body
- `esc()` wordt overal toegepast

- [ ] **Stap 5: Commit en push**

```bash
git add public/index.html
git commit -m "feat(frontend): display cookingTime badge and preparationSteps as numbered list"
